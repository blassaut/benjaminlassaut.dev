...
- On pull requests, a second gate checks the new code rather than the overall score: every mutant on a line the PR adds or changes must be killed (`scripts/mutation-gate.ts`). Failures show up as annotations on the PR diff, and the job summary shows the overall score, the count per status and how many changed-line mutants were checked. A mutant not worth killing is skipped on purpose with `// Stryker disable next-line <Mutator>: <reason>` above the line.
